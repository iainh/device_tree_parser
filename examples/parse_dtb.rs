...
fn parse_header(parser: &DeviceTreeParser) -> Result<(), DtbError> {
    ...
    println!("Magic:           0x{:08x} {}", header.magic, 
        if header.magic == 0xd00d_feed { "✅ Valid" } else { "❌ Invalid" });
...
fn parse_device_tree(parser: &DeviceTreeParser) -> Result<(), DtbError> {
    ...
    println!("Root node: '{}'", if tree.name.is_empty() { "/" } else { tree.name });
    ...
    let total_properties: usize = tree.iter_nodes()
...
fn show_memory_information(tree: &device_tree_parser::DeviceTreeNode) {
    ...
    let memory_nodes: Vec<_> = tree.iter_nodes()
            ...
            if let Some(reg) = memory.prop_u32_array("reg") {
                ...
                for chunk in reg.chunks(2) {
                    if chunk.len() == 2 {
                        ...
                        println!("  Range: 0x{:08x} - 0x{:08x} (size: {} MB)",
                            address,
                            address + size,
                            size / (1024 * 1024)
                        );
...
fn show_device_summary(tree: &device_tree_parser::DeviceTreeNode) {
    ...
    if device_types.is_empty() {
        ...
        for (device_type, count) in sorted_devices {
            println!("  {}: {} instance{}", device_type, count, 
                if count == 1 { "" } else { "s" });
        }
    }
    println!();
...
fn demonstrate_high_level_api(parser: &DeviceTreeParser) -> Result<(), DtbError> {
        ...
        Ok(regions) => {
            if regions.is_empty() {
                ...
                for (i, (addr, size)) in regions.iter().take(5).enumerate() {
                    println!("  Region {}: 0x{:08x} - 0x{:08x} (size: {} bytes)",
    ...
    println!("📊 Nodes with 'interrupts' property: {}", nodes_with_interrupts.len());
